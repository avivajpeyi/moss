...
class SpecVI:
    ...
    def runModel(self, N_delta=30, N_theta=30, lr_map=5e-4, ntrain_map=5e3, inference_size=500, 
        ...
        idx = tf.where(tf.reduce_sum(tf.cast(self.model.trainable_vars[2][0,:,2:] >= 0.1, tf.int32) + tf.cast(self.model.trainable_vars[4][0,:,2:] >= 0.1, tf.int32), -1) == 0)
        for i in idx:
        ...
        return self.spec_mat, Spec_hs, squared_coherences_stats, spectral_density_all, squared_coherences, best_lr_elbo
