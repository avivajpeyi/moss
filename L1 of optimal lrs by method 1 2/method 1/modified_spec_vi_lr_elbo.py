...
class SpecModel(SpecPrep):
    ...
    def loglik_sparse(self, params):
        ...
        ldelta_ = tf.matmul(
            self.Xmat_delta, tf.transpose(params[0], [0, 2, 1])
        )
        ...
        theta_im = tf.matmul(
            self.Xmat_theta, tf.transpose(params[4], [0, 2, 1])
        )

        Z_theta_re_ls = [
            tf.sparse.sparse_dense_matmul(
                self.Z_re[i], tf.transpose(theta_re[:, i])
            )
            - tf.sparse.sparse_dense_matmul(
                self.Z_im[i], tf.transpose(theta_im[:, i])
            )
            for i in range(self.num_obs)
        ]
        Z_theta_im_ls = [
            tf.sparse.sparse_dense_matmul(
                self.Z_re[i], tf.transpose(theta_im[:, i])
            )
            + tf.sparse.sparse_dense_matmul(
                self.Z_im[i], tf.transpose(theta_re[:, i])
            )
            for i in range(self.num_obs)
        ]
        ...
        tmp2_ = -tf.reduce_sum(
            tf.multiply(tf.square(u_re) + tf.square(u_im), delta_inv), [1, 2]
        )
        log_lik = tmp1_ + tmp2_
        return log_lik
    ...
    def runModel(
        ...
        def train_hs(model, optimizer, n_train):
            ...
            for i in tf.range(n_train):
                ...
                if (
                    optimizer.iterations % 500 == 0
                    or optimizer.iterations == 1
                ):
                    tf.print(
        ...
        all_samples = []
        all_losses = []
        lr_range_elbo = np.arange(0.005, 0.1, 0.001)
        for optimizer_vi_val in lr_range_elbo:
            optimizer_vi = tf.optimizers.Adam(optimizer_vi_val)
            losses = tf.function(
        ...
        best_idx = np.argmax(all_losses)
        best_samp = all_samples[best_idx]
        best_lr_elbo = lr_range_elbo[best_idx]
        ...
        spectral_density_q[:, :, diag_indices[0], diag_indices[1]] = (
            np.quantile(
        ...
        squared_coherences_stats = np.percentile(
            squared_coherences, [2.5, 50, 97.5], axis=0
        )
        ...
        return (
            self.spec_mat,
            Spec_hs,
            squared_coherences_stats,
            spectral_density_all,
            squared_coherences,
            best_lr_elbo,
        )
