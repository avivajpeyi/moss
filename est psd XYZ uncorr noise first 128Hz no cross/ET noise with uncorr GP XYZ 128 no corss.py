...
diag_spectral_density_all = np.diagonal(spectral_density_all, axis1=2, axis2=3)  # shape (500, #freq, p)
diag_spec_mat_median = np.diagonal(spec_mat_median, axis1=1, axis2=2)  # shape (#freq, p)
...
def uniformmax_help(sample):
    return np.abs(sample - np.median(sample)) / median_abs_deviation(sample, nan_policy='omit')

mad = median_abs_deviation(diag_spectral_density_all, axis=0, nan_policy='omit')
mad[mad == 0] = 1e-10 
