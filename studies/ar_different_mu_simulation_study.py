"""
Simulate true VAR(2) time series with mean is not 0.
...
class VarmaSim:
    ...
    def calculateSpecMatrix(self, f, varCoef, vmaCoef, sigma=np.array([1.]), inverse=False):
        specTrue = np.apply_along_axis(
            lambda f: self.calculateSpecMatrixHelper(f, varCoef, vmaCoef, sigma, inverse=inverse), axis=1,
            arr=f.reshape(-1, 1))
        return specTrue
...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        ax[i].set_xlim([0, 0.5])
        ax[i].grid(True)
    ax[2].plot(freq, np.absolute(specTrue[:, 0, 1]) ** 2 / (np.real(specTrue[:, 0, 0]) * np.real(specTrue[:, 1, 1])),
