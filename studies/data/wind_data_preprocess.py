...
# take median
median_sknt = mydata.groupby(["station", "lon", "lat", "date", "date2hour"])[
    "sknt"
].median()
...
for s in np.unique(median_sknt_df.station):
    ...
    df.loc[df.station == s, "sknt_diff"] = np.concatenate(
        [
            np.zeros(
                [
                    1,
...
# change long to wide
CA_wide = CA_wind_diff.pivot(
    index="date2hour", columns="station", values="sknt_diff"
)
...
CA_wide_detrend = pd.DataFrame(
    de_stdize, index=CA_wide.index, columns=CA_wide.columns
)
