...
var_coef = np.array([[[0.2, 0.5], [0.0, -0.2]], [[0.0, 0.0], [0.5, -0.2]]])
vma_coef = np.array(
    [[[1.0, 0.0], [0.0, 1.0]], [[0.6, 0], [0.2, -0.5]], [[0.3, 0], [0, 0.3]]]
)
...
result_list = Spec.runModel(
    N_delta=30, N_theta=30, lr_map=5e-4, ntrain_map=5e3, sparse_op=False
)
...
for i in range(2):
    ...
    ax[i].grid(True)
f_CI = (
    np.square(spec_mat[..., 0, 1])
    / np.abs(spec_mat[..., 0, 0])
    / np.abs(spec_mat[..., 1, 1])
)
ax[2].plot(
    freq,
    np.absolute(spec_true[:, 0, 1]) ** 2
    / (np.real(spec_true[:, 0, 0] * np.real(spec_true[:, 1, 1]))),
    linewidth=2,
    color="red",
    linestyle="-.",
    label="Truth",
...
for idx in range(8):
    ...
    ax.set_xlabel(r"$\nu$", fontsize="large")
    ax.set_title(
        r"$\log f_{%d, %d}$ " % (idx + 1, idx + 1), fontsize="x-large", pad=12
    )
    ax.set_xlim([0, 0.5])
    ax.grid(True)
