...
class VarSim:
    def __init__(self, var_coefs, n=1024, sigma=1.0):
        self.n = n
    ...
    def simulateVar(self, Phi, n=1024, sigma=np.array([1.0])):
        ...
        for i in np.arange(lag + 1, x.shape[0]):
            x[i,] = np.sum(
                np.matmul(Phi, x[i - 1 : i - lag - 1 : -1][..., np.newaxis]),
                axis=(0, -1),
            ) + np.random.multivariate_normal(np.repeat(0.0, dim), Sigma)
        x = x[101:]
        return x
    ...
    def calculateSpecMatrix(self, f, inverse=False):
        specMat = []
        for coefs in self.coefs:
            matTemp = np.apply_along_axis(
                lambda f: self.calculateSpecMatrixHelper(
                    f, coefs, sigma=self.sigma, inverse=inverse
                ),
                axis=1,
                arr=f.reshape(-1, 1),
            )
    ...
    def calculateSpecMatrixHelper(self, f, Phi, sigma, inverse=False):
        ...
        A_f_re = np.sum(
            Phi * np.cos(np.pi * 2 * k * f)[:, np.newaxis, np.newaxis], axis=0
        )
        A_f_im = -np.sum(
            Phi * np.sin(np.pi * 2 * k * f)[:, np.newaxis, np.newaxis], axis=0
        )
